...
This part is for storing the dates into the graphs. Throughout the project, I had to repeatedly copy and paste the dates but I discovered you can just save them in the GGPlot data itself, so I am going to store it in there to save space in my code chunks. 
...
B: 4/28 express mission applied https://forums.playlostark.com/t/lost-ark-weekly-update-428-12-am-pt-7-am-utc-9-am-cest/351443

C: 5/6 May update announced https://www.playlostark.com/en-us/news/articles/may-2022-update-reveal 

D: 5/19 May update released https://www.playlostark.com/en-us/news/articles/may-2022-release-notes
...
F: 6/22 June update delayed https://forums.playlostark.com/t/regarding-the-june-update/417483 

G: 6/24 A bit more anti bot measures https://forums.playlostark.com/t/an-update-to-fraud-prevention-in-lost-ark/419449

H: 6/30 June update applied https://www.playlostark.com/en-us/news/articles/wrath-of-the-covetous-legion-release-notes

I: 7/12 hyper express + new class confirmation date https://www.playlostark.com/en-gb/news/articles/arcanist-academy 

J: 7/19 Release of hyper express and arcana https://www.playlostark.com/en-us/news/articles/spells-in-spades-release-notes
...
```{r fig.width= 16}
LAGHLGBPF <- LAGHLGBP + geom_text(data = LAGHLD %>% filter(Date == max(Date), close == last(close)), aes(label = Item), nudge_x = -.8, nudge_y = -5, angle = 45, size = 4) + theme(legend.position = "none")
LAGHLGBPF

LADGGBPF <- LADGBP + geom_text(data = LADGD %>% group_by(Item) %>% filter(Date == max(Date), close == last(close)), aes(label = Item), nudge_x = 7.7, nudge_y = 0, size = 4) + theme(legend.position = "none")
LADGGBPF

LAOrehaDBPF <- LAOrehaBP + geom_text(data = LAOrehaD %>% filter(Date == max(Date), close == last(close)), aes(label = Item), nudge_x = 2, nudge_y = 0, angle = 45, size = 5) + theme(legend.position = "none")
LAOrehaDBPF

LARateupGBPF <- LARateupGBP + geom_text(data = LARateupD %>% group_by(Item) %>% filter(Date == max(Date), close == last(close)), aes(label = Item), nudge_x = 6.5, nudge_y = 0, size = 4) + theme(legend.position = "none")
LARateupGBPF
...
LADGGBPF <- LADGGBPF + theme_minimal() + scale_y_continuous(breaks = seq(0,16,by = 2)) + ylab("Price in gold") + theme_minimal() + theme(legend.position = "none", panel.grid.minor.x = element_blank(), panel.grid.major.x = element_blank())
...
LARateupGBPF <- LARateupGBPF + theme_minimal() + scale_y_continuous(breaks = seq(0,350,by = 20)) + ylab("Price in gold") + theme_minimal() + theme(legend.position = "none", panel.grid.minor.x = element_blank(), panel.grid.major.x = element_blank())

LAShardsGBPF <- LAShardsGBPF + theme_minimal() + scale_y_continuous(breaks = seq(0,550,by = 50)) + ylab("Price in gold") + theme_minimal() + theme(legend.position = "none", panel.grid.minor.x = element_blank(), panel.grid.major.x = element_blank())
