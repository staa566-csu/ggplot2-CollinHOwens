...
LAHoneGBP <- ggplot(data = LAHoneD, mapping = aes(x = Date, y = close, group = interaction(Date, Item), color = Item)) + geom_boxplot()
LAHoneGBP

LARateupGBP <- ggplot(data = LARateupD, mapping = aes(x = Date, y = close, group = interaction(Date, Item), color = Item)) + geom_boxplot()
LARateupGBP

LAShardsGBP <- ggplot(data = LAShardsD, mapping = aes(x = Date, y = close, group = interaction(Date, Item), color = Item)) + geom_boxplot()
LAShardsGBP
```


#Improvements
These are the graphs I would like to use most, so I am going to apply the dates to this graph but also split Oreha material from the Destruction and Guardian stones due to different starting dates of when the Data was collected. 
```{r fig.width= 16}
LAOrehaD <- LostArkData %>% filter(LostArkData$Item %in% c("basic-oreha-fusion-material-2"))
LADGD <- LostArkData %>% filter(LostArkData$Item %in% c("crystallized-destruction-stone-0","crystallized-guardian-stone-0"))
...
LAOrehaDBP <- ggplot(data = LAOrehaD, mapping = aes(x = Date, y = close, group = Date, color = Item)) + geom_boxplot()
LAOrehaDBP <- LAOrehaDBP +
  geom_vline(xintercept= as.Date("2022-04-25"), col="blue") + annotate("text", x = as.Date("2022-04-23"), y = 10, label = "A", col="blue") +
  geom_vline(xintercept= as.Date("2022-04-28"), col="dark green") + annotate("text", x = as.Date("2022-04-29"), y = 10, label = "B", col="dark green") +
  geom_vline(xintercept= as.Date("2022-05-06"), col="blue") + annotate("text", x = as.Date("2022-05-08"), y = 10, label = "C", col="blue") +
  geom_vline(xintercept= as.Date("2022-05-19"), col="dark green") + annotate("text", x = as.Date("2022-05-21"), y = 10, label = "D", col="dark green") +
...
LADGGBP <- ggplot(data = LADGD, mapping = aes(x = Date, y = close, group = interaction(Date, Item), color = Item)) + geom_boxplot()
LADGGBP <- LADGGBP +
  geom_vline(xintercept= as.Date("2022-06-30"), col="dark green") + annotate("text", x = as.Date("2022-06-29"), y = 12, label = "H", col="dark green") +
  geom_vline(xintercept= as.Date("2022-07-12"), col="blue") + annotate("text", x = as.Date("2022-07-11"), y = 10, label = "I", col="blue") +
  geom_vline(xintercept= as.Date("2022-07-19"), col="dark green") + annotate("text", x = as.Date("2022-07-18"), y = 10, label = "J", col="dark green") +
